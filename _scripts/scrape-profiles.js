...
const storeProfileData = async (page, { id, name, rank, rating, average }) => {
  ...
  const pairs = Object.entries({ name, about, rank, rating, average }).map(([k, v]) => {
    return `${k} = ${JSON.stringify(v)}`;
  }).join(',\n');

  await db.run(`UPDATE profiles SET ${pairs} WHERE id = ${id};`);
...
(async () => {
  try {
    ...
    console.log(`Processed ${profiles.length} profiles in ${duration} seconds.`);
